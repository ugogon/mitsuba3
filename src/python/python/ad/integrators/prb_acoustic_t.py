...
class PRBAcoustictIntegrator(RBIntegrator):
    ...
    def sample(self,
        ...
        loop = mi.Loop(name="PRB Acoustic (%s)" % mode.name,
                       state=lambda: (distance, #block.tensor(),
                                      ...
                                      β, η, active,
                                      prev_pi, prev_ray, pi, prev_bsdf_pdf, prev_bsdf_delta))
        ...
        while loop(active):
            ...
            with dr.resume_grad(when=not primal):
                prev_si = prev_pi.compute_surface_interaction(prev_ray, mi.RayFlags.All)
            ...
            if prb_mode and self.track_time_derivatives:
                ...
                if primal:
                    ...
                    with dr.resume_grad():
                        # The surface interaction can be invalid, in which case we don't want it to have any influence
                        τ     = dr.select(active_time, dr.select(first_vertex, si.t, dr.norm(prev_si.p - si.p)), 0.)
                        #τ_dir = dr.select(active_time_next, dr.norm(em_ray.o - si.p), 0.)
                        τ_dir = dr.select(active_time_next, dr.norm(ds.p - si.p), 0.)

                        # attention: the second summand accounts for the direct light segment!
                        δHdTτ = τ * δHdT + τ_dir * δHdTLr_dir
                    δHdT = δHdT - δHdTLe - δHdTLr_dir
            ...
            pi = scene.ray_intersect_preliminary(ray, active=active_next)
            η *= bsdf_sample.eta
            β *= bsdf_weight
...
mi.register_integrator("prb_acoustic_t", lambda props: PRBAcoustictIntegrator(props))
